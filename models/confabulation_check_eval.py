...
from peft import LoraConfig, PeftModel, PeftConfig, LoraModel, get_peft_model, prepare_model_for_kbit_training
import os
import pandas as pd
import re
import json
...
if __name__ == "__main__":
    ...
    model_name = os.path.join(args.result_path, model_name, args.dataset_name,"bert","best")
    ...
    df_merged = df_merged.drop_duplicates().iloc[::100,:]
    ...
    with torch.no_grad():
        outputs = model(tokenized_val_data["input_ids"], attention_mask=tokenized_val_data["attention_mask"])

    # Get the predicted labels
    predicted_labels = outputs.logits.argmax(dim=-1)

    # Get the predicted probabilities
    predicted_probs = torch.softmax(outputs.logits, dim=-1)[:, 1]  # Probabilities for positive class
